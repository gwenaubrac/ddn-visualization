...
# load cohort
cohort <- readRDS(paste(path_temp, 'cohort_cov_and_events.rds', sep = '/'))
...
saveRDS(trt_supply, file = paste(path_intermediate_res, 'trt_supply.rds', sep='/'))
# trt_supply <- readRDS(file = paste(path_intermediate_res, 'trt_supply.rds', sep = '/'))
...
saveRDS(cohort, file = paste(path_temp, 'cohort_cens.rds', sep='/'))
