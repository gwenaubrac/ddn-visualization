...
birth_date <- 
  as.Date('1930-01-01') + sample(0:(as.numeric(as.Date('2000-01-01')) - as.numeric(as.Date('1930-01-01'))), n, replace = TRUE)
...
death_date <- sapply(birth_date, function(birth_date) {
  if (runif(1) < 0.3) { # setting a 30% probability of death
    ...
    if (death_date < birth_date) {
      # if death prior to entry artifically set to some years after birth
      death_date <- birth_date + sample(30:90, 1) * 365 
    }
...
for (patient_id in patient$id) {
  ...
  rx_dates[index] <- 
    as.Date('2018-01-01') + sample(0:(as.numeric(as.Date('2023-01-01')) - as.numeric(as.Date('2018-01-01'))), 1)
...
# 1. Patient only has 1 refill (single refill), so medication supply ends at end of refill
...
d) **marg_bias:** marginal bias terms (only for binary cov)
...
cohort_analytic_sens$at_exit_date <- pmin(
  ...
  # this is what differs -- using disc date from 90-day grace period:
...
# specify multi-category variables and demographic variables
# (for which we won't calculate marginal bias)
skip_var <- c("age_group", "male")
...
get_pt_per_month <- function(month, data) {
  ...
  data[[month_col]] <- case_when(
    # if patient entered after month ended OR entered and left before month started
    # -> 0 days contributed
    ...
    # if patient entered on/before month started and left on/after month ended
    # -> entire month contributed
    data$entry_date <= month_start &
      data$itt_exit_date >= month_end ~ days_in_month(ym(month)),
    
    # if patient entered on/before month started and left before month ended
    # -> days before left contributed
    data$entry_date <= month_start &
      data$itt_exit_date < month_end ~ as.numeric(data$itt_exit_date - month_start),
    
    # if patient entered after month started and left on/after month ended
    # -> days since entered contributed
    data$entry_date > month_start &
      data$itt_exit_date >= month_end ~ as.numeric(month_end - data$entry_date),
    
    # if patients entered after month started and left before month ended
    # -> days between entry and exit contributed
    data$entry_date > month_start &
