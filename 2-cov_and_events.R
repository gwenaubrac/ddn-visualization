...
## (for which only the HES date is used).
##
## Author: Gwen Aubrac
##
...
# load cohort
cohort <- readRDS(paste(path_temp, 'cohort_creation.rds', sep = '/'))
...
saveRDS(cprd_event, file = paste(path_temp,  'cprd_event_A1.rds', sep='/'))
rm(cprd_event)

gc()
cprd_event <- data.frame()
mclapply(observation_filesA2, filter_event)
saveRDS(cprd_event, file = paste(path_temp,  'cprd_event_A2.rds', sep='/'))
rm(cprd_event)

gc()
cprd_event <- data.frame()
mclapply(observation_filesA3, filter_event)
saveRDS(cprd_event, file = paste(path_temp,  'cprd_event_A3.rds', sep='/'))
rm(cprd_event)
...
saveRDS(cprd_event, file = paste(path_temp, 'cprd_event_B1.rds', sep='/'))
rm(cprd_event)

gc()
cprd_event <- data.frame()
mclapply(observation_filesB2, filter_event)
saveRDS(cprd_event, file = paste(path_temp, 'cprd_event_B2.rds', sep='/'))
rm(cprd_event)

gc()
cprd_event <- data.frame()
mclapply(observation_filesB3, filter_event)
saveRDS(cprd_event, file = paste(path_temp, 'cprd_event_B3.rds', sep='/'))
rm(cprd_event)
...
saveRDS(cprd_event, file = paste(path_temp, 'cprd_event_C.rds', sep='/'))

rm(cprd_event)

cprd_event_A1 <- readRDS(file = paste(path_temp, 'cprd_event_A1.rds', sep = '/'))
cprd_event_A2 <- readRDS(file = paste(path_temp, 'cprd_event_A2.rds', sep = '/'))
cprd_event_A3 <- readRDS(file = paste(path_temp, 'cprd_event_A3.rds', sep = '/'))
cprd_event_B1 <- readRDS(file = paste(path_temp, 'cprd_event_B1.rds', sep = '/'))
cprd_event_B2 <- readRDS(file = paste(path_temp, 'cprd_event_B2.rds', sep = '/'))
cprd_event_B3 <- readRDS(file = paste(path_temp, 'cprd_event_B3.rds', sep = '/'))
cprd_event_C <- readRDS(file = paste(path_temp, 'cprd_event_C.rds', sep = '/'))
...
# for hospitalizations for suicidal ideation, hypoglycemia, and CHF,
# we will only consider inpatient events identified from HES
# so remove these from CPRD events
cprd_event %<>% select(id, event_name, date)
cprd_event %<>% filter(!event_name %in% c('suicidal_hosp', 'hypoglycemia_hosp', 'chf_hosp'))
...
# make sure all_event has obs for suicidal and hypoglycemia
...
saveRDS(cohort, file = paste(path_temp, 'cohort_cov_and_events.rds', sep='/'))
